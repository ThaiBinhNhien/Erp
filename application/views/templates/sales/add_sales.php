...
<div class="wrapper-contain add-revenues order">
	...
	<div class="row sec-row">
		<table class="add-table margin-top-table first-add-table ">
		    <tbody>
		            <th style="width: 17%;">ﾘﾈﾝｻﾌﾟﾗｲ売上</th>
		            <th style="width: 15%;" class="col_2_name">
		            	<?php
		            	if($environment_tax > 0 & $sum_price_cate1 > 0) echo "ﾘﾈﾝ補充費";
		            	elseif($discount != 0 & $sum_price_cate1 > 0) echo "値引分".abs($discount)."%"; 
		            	?>
		            </th>
		            <th style="width: 17%">ｸﾘｰﾆﾝｸﾞ他売上</th>
		            <th style="width: 17%">小　　　　計</th>
		            <th style="width: 17%" class="tax_name">
		            	<?php if($tax > 0) echo '消費税'.$tax.'%'; ?>
		            </th>
		            <th style="width: 17%">請求金額（税込）</th>
		            <tr>
		                <td class="right-text sum_price_cate1"><?php if($sum_price_cate1>0) echo $sum_price_cate1; ?></td><!--hàng khăn-->
		                <td class='right-text col_2_value'>
		                	<?php if(($environment_tax != 0 | $discount != 0) & $sum_price_cate1>0)
		                	echo abs((int)$environment_tax+(int)$discount)*$sum_price_cate1/100; 
		                	?>
		                </td>
		                <input type="hidden" id="col_2_hide_value" value="<?php echo((int)$environment_tax+(int)$discount); ?>">
		                <td class="right-text sum_price_cate2"><?php if($sum_price_cate2>0) echo $sum_price_cate2; ?></td><!--hàng giặc-->
		                <?php $sum_price = $sum_price_cate1 + $sum_price_cate2 + ((int)$environment_tax+(int)$discount)*$sum_price_cate1/100; ?>
...
<table class="add-table first-add-table table_department">
    <tbody>
    	...
    	<input type="hidden" class="department_id_cate1" value="aaa">
        ...
        <tr>
            ...
            <td><?php echo $product->{DD_UNIT_PRICE}+0 ?></td> 
        ...
        <tr>
        	...
        	<td></td>
        	<td></td>
        	<td><input type="text" style="text-align: right;" class="discount_cate1 input" value="0"></td>
        </tr>
    </tbody>
</table>  								
</div>
...
<table class="add-table first-add-table table_department">
    <tbody>
    	...
    	<input type="hidden" class="department_id_cate2" value="aaa">
        ...
        <tr>
        	...
        	<td></td>
        	<td></td>
        	<td><input type="text" style="text-align: right;" class="discount_cate2 input" value="0"></td>
        </tr>
    </tbody>
</table>		
</div>
...
<?php } ?>
<div class="row first-row">
	<a class="print print-1 left back">戻る</a>
