...
<head>
	...
	<style type="text/css">
	/*@page {
		odd-header-name: 'dfsdrfdfdf';
        even-header-name: 'dfsdfdsfsdfd';
		margin: 70px;
...
<table class="table" width="100%">
	<tr class="thead">
		<td width="14%"><?php if($sum_price_cate1>0) echo 'ﾘﾈﾝｻﾌﾟﾗｲ売上'; ?></td>
		<td width="13%">
			<?php 
		...
		<td width="14%"><?php if($sum_price_cate2>0) echo 'ｸﾘｰﾆﾝｸﾞ他売上'; ?></td>
		<td width="14%">管理業務料</td>
		<td width="15%">小　計</td>
		<td width="15%"><?php if($tax>0) echo('消費税'.$tax.'%')?></td>
		<td width="15%">請求金額（税込）</td>
	</tr>
	<tr>
		<td>
			<?php 
		...
		<td>
			...
			$discount_environment = $sum_price_cate1*$environment_tax*$discount_gaichyu->environment_discount/10000;
			?>
		</td>
		<td><?php 
		...
		<td>
		<?php 
		$sum_price = $sum_price_cate1 + $sum_price_cate1*$col_2_value/100 + $sum_price_cate2 - $fee_gaichyu;
...
<table width="100%" class="table_">
	...
	<tr>
		...
		<td><?php echo($product->{DD_GAICHYU_PRICE}) ?></td>
		<td><?php echo number_format($product->{DD_QUANTITY}*$product->{DD_GAICHYU_PRICE}) ?></td>
		<?php $sum_price += $product->{DD_QUANTITY}*$product->{DD_GAICHYU_PRICE}; ?>
	</tr>
	<?php } ?>
...
<table width="100%" class="table_">
	...
	<tr>
		...
		<td><?php echo($product->{DD_QUANTITY}) ?></td>
		<td><?php echo($product->{DD_GAICHYU_PRICE}) ?></td>
		<td><?php echo($product->{DD_QUANTITY}*$product->{DD_GAICHYU_PRICE}) ?></td>
		<?php $sum_price += $product->{DD_QUANTITY}*$product->{DD_GAICHYU_PRICE}; ?>
	</tr>
	<?php } ?>
