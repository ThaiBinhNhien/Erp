...
class Setting_ProductController extends VV_Controller {
    ...
    { 
		...
		$this->load->model('ProductSetProduct','product_set_product_model');
		$this->load->model('ImportExportCsv');
	}

	public function index() {
		$data['title'] = $this->lang->line('ms_setting_product');
        $data['content'] ='masters/setting_product/index';
        $this->load->view('templates/master',$data);
    }
    
    // Add
	public function add() {
		$data['title'] = $this->lang->line('ms_add_setting_product');
        $data['content'] ='masters/setting_product/add'; 
        $this->load->view('templates/master',$data);
    }
    
    // Edit
	public function edit() {
		$data['title'] = $this->lang->line('ms_edit_setting_product');
		$id_set_product = $this->input->get("id");
		$where_set_product[PS_PRODUCT_SET_CODE] = $id_set_product;
        ...
        $data['content'] ='masters/setting_product/edit';
        $this->load->view('templates/master',$data);
	}
    ...
    public function add_post(){ 
        ...
        if($this->input->server("REQUEST_METHOD") == "POST"){
			...
			try{
				...
				$checkIsExits = $this->product_set_model->isExitsRow(PS_PRODUCT_SET_CODE,$id_set_product );
				...
				// Meta
				$arr_add[PS_PRODUCT_SET_CODE] = $id_set_product;
				$arr_add[PS_PRODUCT_SET_NAME] = $name;
				...
				if($resultProduct > 0) {
					foreach ($detail as $key => $value) {
						$arr_add_set_product[PSP_PRODUCT_SET_CODE] = $id_set_product;
						$arr_add_set_product[PSP_PRODUCT_CODE] = $value["id"];
						$arr_add_set_product[PSP_SERIAL_NUMBER] = $value["stt"];
				...
				{
					// LOG ADD
					logadd(PS_PRODUCT_SET_CODE . ":".$id_set_product, PRODUCT_SET);
    ...
    public function edit_post(){ 
        ...
        if($this->input->server("REQUEST_METHOD") == "POST"){
			...
			try{
				...
				// Meta
                $arr_edit[PS_PRODUCT_SET_NAME] = $name;
                $arr_where[PS_PRODUCT_SET_CODE] = $id_set_product;
				$this->product_set_model->editByWhere($arr_where, $arr_edit);

				// Detail
				$resultProduct = count($detail);
				$arr_where_set_product[PSP_PRODUCT_SET_CODE] = $id_set_product;
				...
				else
				{
					...
					logedit($data_log_edit, PRODUCT_SET);
    ...
    */
    public function delete_post(){ 
        ...
        if($this->input->server("REQUEST_METHOD") == "POST"){
			...
			try{
				...
				// Meta
                $arr_where[PS_PRODUCT_SET_CODE] = $id;
				$this->product_set_model->removeByWhere($arr_where);

				$where_dup_detail[PSP_PRODUCT_SET_CODE] = $id;
				...
				{
					// LOG DELETE
					logdelete(PS_PRODUCT_SET_CODE . ":".$id, PRODUCT_SET);
					$this->product_set_model->db->trans_commit();
					$this->product_set_product_model->db->trans_commit();
					echo json_encode(array(
						"success" => true,
        ...
        try{
			if($get_type_csv == 1 || $get_type_csv == "1") {
				...
				foreach ($sheetData as $key => $value) {
					$item = array();
					$item[PS_PRODUCT_SET_CODE] = (isset($value['A']) ? $value['A'] : null);
					...
					$item[PS_PRODUCT_SET_NAME] = (isset($value['B']) ? $value['B'] : null);

					$where_dup[PS_PRODUCT_SET_CODE] = (isset($value['A']) ? $value['A'] : null);
					...
					if ($this->product_set_model->db->trans_status() === FALSE){
						...
						$where_dup_detail[PSP_PRODUCT_SET_CODE] = (isset($value['A']) ? $value['A'] : null);
						$this->product_set_product_model->removeByWhere($where_dup_detail);
					}
					$index = $key + 1;  						
				}
				...
				foreach ($sheetData as $key => $value) {
					$item = array();
					$item_detail[PSP_PRODUCT_SET_CODE] = (isset($value['A']) ? $value['A'] : null);
                    $item_detail[PSP_PRODUCT_CODE] = $value["B"];
					$item_detail[PSP_SERIAL_NUMBER] = $value["C"];
    ...
    public function export(){
		...
		if($type == 1 || $type == "1") {
			$title = $this->lang->line('ms_setting_product');
			$column_title = array(PS_PRODUCT_SET_CODE,PS_PRODUCT_SET_NAME);
			$column_show_data = array(PS_PRODUCT_SET_CODE,PS_PRODUCT_SET_NAME);
	
			$result = $this->product_set_model->getAll();

			$this->ImportExportCsv->export($title, $column_title, $column_show_data,$result);
		} else if($type == 2 || $type == "2") {
			$title = PRODUCT_SET_PRODUCT;
			$column_title = array(PSP_PRODUCT_SET_CODE,PSP_PRODUCT_CODE,PSP_SERIAL_NUMBER);
			$column_show_data = array(PSP_PRODUCT_SET_CODE,PSP_PRODUCT_CODE,PSP_SERIAL_NUMBER);
