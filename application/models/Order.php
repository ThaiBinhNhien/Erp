...
class Order extends VV_Model 
{
	...
	// Count order by role
	public function getCountCheckRole($id_order)
	{ 
		$customer_id = $this->customer_account == null? null:$this->customer_account[CUS_ID];
		$is_customer = $this->customer_account == null? 0:1;

		$query = "
		SELECT COUNT(O.`".SL_ID."`) FROM `".SALES_LEDGER."` O
		";
		...
		if($id_order != NULL){
			$whereClause .= ($whereClause == "WHERE "?"":"AND ");
			$whereClause .= "O.`".SL_ID."`=$id_order ";
		}
		
		if($this->level == "P" && $is_customer == 0){
			$whereClause .= ($whereClause == "WHERE "?"":"AND ");
			$whereClause .= "O.`".SL_DEPARTMENT_CODE."` IN ( SELECT `".CUSTOMER_DEPARTMENT."`.`".CD_DEPARTMENT_CODE."` FROM `".CUSTOMER_DEPARTMENT."` WHERE `".CD_CUSTOMER_ID."`=O.`".SL_CUSTOMER_ID."` AND `".CD_USER_ID."`='".$this->LOGIN_INFO[U_ID]."')";
			//$whereClause .= "O.`".SL_CUSTOMER_ID."` IN ( SELECT `".CUSTOMER_DEPARTMENT."`.`".CD_CUSTOMER_ID."` FROM `".CUSTOMER_DEPARTMENT."` WHERE `".CD_USER_ID."`='".$this->LOGIN_INFO[U_ID]."')";
		}

		$whereClause = $whereClause=="WHERE "?"":$whereClause;
		$query .= $whereClause." GROUP BY O.".SL_ID;
	...
	{ 
		...
		if($this->level == "P" && $is_customer == 0){
			$whereClause .= ($whereClause == "WHERE "?"":"AND ");
			//$whereClause .= "o.`".SL_CUSTOMER_ID."` IN ( SELECT `".CUSTOMER_DEPARTMENT."`.`".CD_CUSTOMER_ID."` FROM `".CUSTOMER_DEPARTMENT."` WHERE `".CD_CUSTOMER_ID."`=o.`".SL_CUSTOMER_ID."` AND `".CD_USER_ID."`='".$this->LOGIN_INFO[U_ID]."')";
			$whereClause .= "o.`".SL_DEPARTMENT_CODE."` IN ( SELECT `".CUSTOMER_DEPARTMENT."`.`".CD_DEPARTMENT_CODE."` FROM `".CUSTOMER_DEPARTMENT."` WHERE `".CD_CUSTOMER_ID."`=o.`".SL_CUSTOMER_ID."` AND `".CD_USER_ID."`='".$this->LOGIN_INFO[U_ID]."')";
		}
		$whereClause = $whereClause=="WHERE "?"":$whereClause;
	...
	public function checkOwner($order_id,$customer_id){
		$query = "sELECT * FROM `".SALES_LEDGER."` S WHERE S.`".SL_ID."`=".$order_id." AND ";
		if($customer_id != null){
			$query .= "S.`".SL_CUSTOMER_ID."`=$customer_id ";
		}else{
			$query .= "S.`".SL_DEPARTMENT_CODE."` IN ( SELECT `".CUSTOMER_DEPARTMENT."`.`".CD_DEPARTMENT_CODE."` FROM `".CUSTOMER_DEPARTMENT."` WHERE `".CD_CUSTOMER_ID."`=S.`".SL_CUSTOMER_ID."` AND `".CD_USER_ID."`='".$this->LOGIN_INFO[U_ID]."')";
		}
		return $this->getQuery($query);
	}
