    ...
    $.ajax({
        ...
        success:function(result){
            ...
            for(var i=0; i<result.length; i++){
                ...
                if(result[i]['type'] != "" && result[i]['type'] != null) {
                    html += '<td>'+result[i]['type']+'</td>';
                } else {
                    html += '<td></td>';
                }
                if(result[i]['type_name'] != "" && result[i]['type_name'] != null) {
                    html += '<td>'+result[i]['type_name']+'</td>';
                } else {
                    html += '<td></td>';
                }
                html += '<td><a data-type="'+result[i]['type']+'" data-weight="'+result[i]['weight']+'" data-id="'+result[i]['id']+'" data-name="'+result[i]['name']+'" class="btnEdit"><img src="'+urlImage+'edit.png"/></a>  <a data-id="'+result[i]['id']+'" class="btnDelete"><img src="'+urlImage+'del.png"/></a></td>';
                html += '</tr>';
            }
...
$(document).on("click","#btnAddCategory", function(){
    ...
    $('#group_type').rules('add', { required: true });
    $('#group_name').tooltip({
        trigger: 'manual',
        placement:'top'
    });
    ...
    $(this).helloWorld(message_confirm_save_field ,null,null,
        {
            url:addPostLink,
            data:{id:id,name:name,weight:weight,type:type },
...
$(document).ready(function(){
	window.add_record = function add_record(data,result){
        if(result.success == true) {
            ...
            var name = $("#group_name").val();
            var weight = $("#group_weight").val();
            var type_name = $("#group_type option:selected").text();
            var type_id = $("#group_type option:selected").val();
            ...
            html += '<td><a data-type="'+type_id+'" data-weight="'+weight+'" data-id="'+id+'" data-name="'+name+'" class="btnEdit"><img src="'+urlImage+'edit.png"/></a>  <a data-id="'+id+'" class="btnDelete"><img src="'+urlImage+'del.png"/></a></td>';
			html += "</tr>";
	...
	window.edit_record = function edit_record(data,result){ 
            ...
            editTr.find("td:eq(2)").text(weight);
            editTr.find("td:eq(3)").text(type_id);
            editTr.find("td:eq(4)").text(type_name);
    }
});