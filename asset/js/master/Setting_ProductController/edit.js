...
function LoadData(){
	$.ajax({
		url:productViewUrl,
		data:{type_product:3,product_id:"",supplier:"",product_name:"",t_category:"",
		...
		success:function(result){
			...
			$('#tab_product_set tbody tr').each(function(){
				...
				detail_arr.push({"product_id":id_product});
			});

			var html = '';
			for(var i=0; i<result.length; i++){
				var is_html = true;
				for(j=0; j<detail_arr.length; j++) {
					if(result[i]['id'] == detail_arr[j]['product_id']) {
						is_html = false;
					}
				}
				if(is_html == true) {
					...
					html += '<tr id="product'+result[i]['id']+'" data-id='+result[i]['id']+' style="display: none;">';
					html += '<td class="col-xs-3">'+result[i]['sell_product_id']+'</td>';
					html += '<td class="col-xs-7">'+result[i]['sell_product_name']+'</td>';
					html += '</tr>';
				}
						...
						$('#tab_product_set tbody tr').each(function(){
						  ...
						  for(var i=0; i<result.length; i++){
							...
							if(is_html == true) {
								...
								var html = '<tr id="product'+result[i]['id']+'" data-id='+result[i]['id']+' style="display: none;">';
								html += '<td class="col-xs-3">'+result[i]['sell_product_id']+'</td>';
								html += '<td class="col-xs-7">'+result[i]['sell_product_name']+'</td>';
								html += '</tr>';
								$('#tab_product tbody').append(html);
 ...
 $(document).on("click","#right", function(){
    $('#tab_product tbody tr.active').each(function(){
        ...
        html += $(this).html();
        html += '<td class="col-xs-2 input-fixed"><input class="form-control valid_stt_product"  name="form_stt" id="form_stt'+id_product+'"></td>';
        html += '</tr>';
 ...
 // Add 
$(document).on("click","#btnEditSetProduct", function(){
	...
	$("input.valid_stt_product").each(function () {
                $(this).rules("add", {
                    required: true,
                    number: true, 
                    min: 0,
	...
	$(this).helloWorld(message_confirm_save_field ,urlIndex,null,
		{
			url:editListSetProduct,
			data:{id:id_set_product,name:name_set_product,detail:detail_arr },
