...
function showList(val_async){
    ...
    var type = $('#type_product option:selected').val();
    ...
    $.ajax({
        ...
        data:{id:id_search,name:input_search,type:type},
        async:val_async,
        dataType:'json',
        method:'GET',
        success:function(result){
            ...
            for(var i=0; i<result.length; i++){
                ...
                if(result[i]['type'] == 1 || result[i]['type'] == "1") {
                    html += '<td>リネンサプライ</td>';
                ...
                html += '<td><a data-id="'+result[i]['id']+'" data-outsource="'+result[i]['flg_outsource']+'" data-type="'+result[i]['type']+'" data-name="'+result[i]['name']+'" class="btnEdit"><img src="'+urlImage+'edit.png"/></a>  <a data-id="'+result[i]['id']+'" class="btnDelete"><img src="'+urlImage+'del.png"/></a></td>';
                html += '</tr>';
            }
...
function renewScroll(){
    $('#list-table .dataTables_scrollBody').on('scroll', function() {
        ...
        if($(this)[0].scrollHeight - $(this).scrollTop() >= ($(this).outerHeight() - 20) && $(this)[0].scrollHeight - $(this).scrollTop() <= $(this).outerHeight()) {
            ...
            $.ajax({
                url:getListCatelogue,
                data:{id:id_search,name:input_search,type:type,start_index:start_index},
                dataType:'json',
                method:'GET',
                success:function(result){
...
$(document).on("click",".btnDelete",function(){
    ...
    var mesdel = String.format(message_confirm_delete_field, $('table thead tr th:first').html()+" : "+value_id);
    $(this).parent().parent().attr("data-delete","1");
	$(this).parent().parent().siblings().attr("data-delete","0"); 
    $(this).helloWorld(mesdel, null , null, { 
        url: deleteCatelogue,
        data:{id:value_id},
...
$(document).on("click","#btnInsert",function(){
    $("form .tooltip").remove();
    $("form#form_add_category")[0].reset();

    var type = $('input[name=optradioType]:checked').val();
    $('#create_type_product').show();
    $('input[name=optradioTypeInsert][value='+type+']').prop('checked', true);
    $('#group_name').val("");
    $('#group_id').val("");
...
$(document).on("click",".btnEdit",function(){ 
    ...
    var flg_outsource = $(this).data("outsource");
    if(flg_outsource == 1 || flg_outsource == "1") {
        flg_outsource = 2;
    } else {
        flg_outsource = 1;
    }
    ...
    if(type == 2 || type == "2") {
        $('#create_type_product').hide();
    } else {
        $('#create_type_product').val(flg_outsource);
        $('#create_type_product').show();
    }
...
$(document).on("click","#btnAddCategory", function(){
    ...
    var type = $('input[name=optradioTypeInsert]:checked').val();
    var type_product = $("#create_type_product option:selected").val();

    // Ajax Add-Popup
    $(this).helloWorld(message_confirm_save_field ,null,null,
        {
            url:addPostCatelogue,
            data:{id:id,name:name,type:type,type_product:type_product },
...
$(document).on("change","input[name=optradioType]", function(){
    showList();
});
...
$(document).ready(function(){
	window.add_record = function add_record(data,result){
        if(result.success == true) {
            ...
            html += '<td><a data-id="'+id+'" data-outsource="'+type_product+'" data-type="'+type+'" data-name="'+name+'" class="btnEdit"><img src="'+urlImage+'edit.png"/></a>  <a data-id="'+id+'" class="btnDelete"><img src="'+urlImage+'del.png"/></a></td>';
			html += "</tr>";
	...
	window.edit_record = function edit_record(data,result){ 
            ...
            editTr.find("td:eq(2)").text(type_name);
            editTr.find(".btnEdit").data("type",type);
            editTr.find(".btnEdit").data("name",name);
            editTr.find(".btnEdit").data("outsource",type_product);
    }
});

$(document).on("change","input[name=optradioTypeInsert]",function(){
    var value_type = $(this).val();
    if(value_type == 1 || value_type == "1") {
        $('#create_type_product').show();
    } else {
        $('#create_type_product').val(1);
        $('#create_type_product').hide();
    }
});
