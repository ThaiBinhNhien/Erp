...
(function(factory) {
    ...
    methods = {
            ...
            return this.each(function () {
                ...
                }
                {
                    ...
                    input.on('focus.inputpicker', _eventFocus)
                        ...
                        //.on('click.inputpicker', _eventKeyUp);
        ...
        destroy: function (options) {
            return this.each(function () {
                ...
                if (typeof original_css != 'undefined') {
    ...
    function _execJSON(input, param, func) {
        ...
        for(var propertyName in param) {
            if($.isNumeric(param[propertyName]) == false){
                var replace = input.val();
                if(replace == ''){
                    replace = param.value;
                }
                param[propertyName] = param[propertyName].replace("{{q}}",replace);
            }
        }
    ...
    function _renderPaginationFooter(input){
        ...
        if( fields.length > 0 && _pagination(input)){
            ...
            output += "<div style=\"float:left;padding-left:5px;\">検索結果が" + count + "件あります。</div>";

            if(count > limit) {
                output += "<div style=\"float:right;padding-right:5px;\">" +
                    "<a href=\"javascript:void(0);\" onclick=\"$('#inputpicker-" + _uuid(input) + "').inputpicker('jumpToPage', '1');\">最初</a>" +
                    "<a href=\"javascript:void(0);\" onclick=\"$('#inputpicker-" + _uuid(input) + "').inputpicker('jumpToPage', '" + prev_page + "');\">前</a>" +
                    "<span class=\"current-page\">" + current_page + "</span>" +
                    "<a href=\"javascript:void(0);\" onclick=\"$('#inputpicker-" + _uuid(input) + "').inputpicker('jumpToPage', '" + next_page + "');\">次</a>" +
                    "<a href=\"javascript:void(0);\" onclick=\"$('#inputpicker-" + _uuid(input) + "').inputpicker('jumpToPage', '" + last_page + "');\">最後</a>" +
                    "</div>";
            }
        ...
        if(data.length){
            ...
            for(var i = 0; i < data.length; i++) {
                ...
                for (var j = 0; j < fields.length; j++) {
                    ...
                    if(input_keyword_low != null && input_keyword_low != "" && text != "" && text != null) {
    ...
    function _matchActiveInRender(input) {
        ...
        for(var x = 0; x < elements.length; x++){
            ...
            for (var j = 0; j < fields.length; j++) {
                ...
                var field_name = (typeof fields[j] == 'object') ? fields[j]['name'] : fields[j];
                if (typeof data[i][field_name] == 'undefined' || data[i][field_name] == null){
                    continue;
                }
